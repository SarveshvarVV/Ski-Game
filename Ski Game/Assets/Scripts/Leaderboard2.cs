...
using UnityEngine;
using TMPro; // for Course 4 Session 4
using DG.Tweening; // for Course 4 Session 4

public class Leaderboard2 : MonoBehaviour
{
    public string[] formattedTimes;
    public TextMeshProUGUI[] highScoreText;//// for Course 4 Session 4
    ...
    private void CheckRaceTime()
    {
        ...
        for (int i = 4; i >= 0; i--)
        {
            ...
            if (RaceTimer2.time < savedTimes[i]||savedTimes[i]==0)
            {
                highScore = true;
        ...
        if (highScore)
        {
            savedTimes.Insert(scorePosition, RaceTimer2.time);
            SetBestTimes();
            //highlight the new highscore
            highScoreText[scorePosition].color = new Color(0.75f, 0, 1);//// for Course 4 Session 4
            highScoreText[scorePosition].DOFade(0.1f, 0.25f).SetLoops(-1, LoopType.Yoyo);//// for Course 4 Session 4
    ...
    private void FormatTimesToString()
    {
        for (int i = 4; i >= 0; i--)
        {
            ...
            highScoreText[i].text = t.ToString("m':'s':'ff"); //// for Course 4 Session 4
